...
print FILE "\t(\n";
foreach $key (sort keys %signals_hash){
    @sorted_vals = sort values %{$signals_hash{$key}}; # sort items of inner hash by value
    #print "@sorted_vals\n";
    foreach $k (@sorted_vals){
	if (($key eq (keys %signals_hash)[-1]) && ($k eq $sorted_vals[-1])){ # if the current item is the very last item in the nested hashes
	    print FILE "\t$key\t$k\n";
	} else {
	    print FILE "\t$key\t$k,\n";
	}
