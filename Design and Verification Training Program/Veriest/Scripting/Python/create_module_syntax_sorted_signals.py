...
od = collections.OrderedDict(sorted(d.items())) # Sort d items by key
for key in od:
    sort_by_val = sorted(od[key].items(), key = lambda x: x[1]) # Sort od[key] items by value
    for i in sort_by_val:
        if ((key == od.keys()[-1]) & (i == sort_by_val[-1])): # if the current item is the very last item in the nested dicts
            f.write("\t" + key + "\t" + i[1] + "\n") 
        else:
            f.write("\t" + key + "\t" + i[1] + ",\n")
