...
// test_uvc_seq - sends packets to all 4 channels with incrementing payloads from 1 to 22.
class test_uvc_seq extends yapp_base_seq;
	...
	// sequence body definition
	// we can't use randomization as the stimulus requires packets to be sent to the illegal address 3.
	virtual task body();
		`uvm_info(get_type_name(), "Executing TEST_UVC_SEQ", UVM_LOW)
		`uvm_create(req) // create YAPP packet instance
		req.packet_delay = 1;
		// nested loops to set the address from 0 to 3, and the length from 1 to 22.
		for(int ad = 0; ad < 4; ad++) begin // address loop
			req.addr = ad;
			for(int lgt = 1; lgt < 23; lgt++) begin // length loop
				req.length = lgt;
				req.payload = new[lgt]; // without randomization, we need to manually create the payload dynamic array. 
				for(int pld =0; pld < lgt; pld++)
					req.payload[pld] = pld;
				randcase // we use randcase to randomly select good or bad parity
					20 : req.parity_type = BAD_PARITY;
					80 : req.parity_type = GOOD_PARITY;
				endcase
				req.set_parity();
				`uvm_send(req) // sending each packet
			end // length loop
		end // address loop
	endtask
